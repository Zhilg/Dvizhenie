...
content-type: application/json (обязательный)
x-corpus-path: относительный путь к каталогу с файлами. Путь указывается относительно точки монтирования докера ПМ (обязательный). Точка монтирования - папка shared_data. Корпус должен быть предзагружен с использованием /semantic/upload.
x-model-id: идентификатор модели (обязательный)
x-ttl-hours: количество часов, по истечении  которых можно удалять производные данные. 0 или не указано - не удалять. (необязательный)
