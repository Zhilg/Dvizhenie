...
1. Чтение иерархии файлов в файловой системе. Возможные типы файлов:
 Текстовые. Формат: txt - кодировка - UTF-8
2. Векторизация файлов с использованием указанной модели
3. Кластеризация файлов
...

{
  ...
  "data": {
    ...
    "children": [                     # Список ОСНОВНЫХ кластеров 1-го уровня
      {
        "id": "cluster1",
        "name": "Технологии",         # Название кластера
        "fileCount": 12480,            # Количество файлов в этом кластере
        "avgSimilarity": 0.87,         # Средняя близость внутри кластера
        "similarityDistribution": [0, 0.1, 0.2, 0.3, 0.4], # Распределение близости (Всегда возвращается массив из 5 элементов. Массив [0, 0.1, 0.2, 0.3, 0.4] - означает, что 0% файлов лежат в промежутке 0 - 0.2 по коэффициентам близости, 10% - в промежутке 0.2 - 0.4 и т.д.)
        "files": [
          {
            "name": "ai_research.txt", # Имя файла
            "similarity": 0.92, # Коэффициент близости этого файла к кластеру
            "size": 1024, # Размер файла в байтах
            "date": "2023-01-15T00:00:00Z" # Дата создания файла
          }, ...
        ],               # Файлы, напрямую входящие в этот кластер
        "children": [...]             # ПОДкластеры (например, "ИИ" внутри "Технологий"), структура такая же
      },
      {
        "id": "cluster2",
        "name": "Наука",
