...
x-corpus-id: uuid, идентификатор корпуса, загруженного через /semantic/upload (обязательный). Используется то значение, что вернул контейнер разработчика после выполнения загрузки.
